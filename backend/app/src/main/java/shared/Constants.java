package shared;

public class Constants {

    public static final String MAIN_ACTIVITY_TYPE = "main";

    public static final String LOG_MAIN = "WHACK-MAIN";

    public static final String PING = "PING";
    public static final String GAME_START = "GAME_START";
    public static final String CLICKED_BEER = "CLICKED_BEER";
    public static final String CONFIG = "CONFIG";

    public static final String PLAYER_NAME = "playerName";
    public static final String SERVER_NAME = "lobbyName";
    public static final String SERVER_PASSWORD = "lobbyPassword";

    public static final String LOG_NETWORK = String.format("%s-Network", LOG_MAIN);
    public static final String LOG_ERROR = String.format("%s-Error", LOG_MAIN);

}
